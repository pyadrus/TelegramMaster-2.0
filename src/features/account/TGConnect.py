...
class TGConnect:
    ...
    async def get_telegram_client(self, page, session_name, account_directory):
        ...
        """
        logger.info(f"Подключение к аккаунту: {account_directory}/{session_name}. Используем API ID: {self.api_id}, API Hash: {self.api_hash}")  # Имя файла сессии file[0] - session файл
            ...
            async def btn_click(e: ft.FilePickerResultEvent) -> None:
                ...
                if e.files:
                    ...
                    if file_name.endswith(".session"):
                        ...
                        target_folder = f"user_data/accounts/{account_directory}"
