...
class CreatingGroupsAndChats:
    ...
    async def creating_groups_and_chats(self, page: ft.Page) -> None:
        ...
        async def add_items(_):
            ...
            try:
                for session_name in await find_filess(directory_path=path_creating_folder, extension='session', list_view=list_view, page=page):
                    client = await self.tg_connect.get_telegram_client(page, session_name,
                                                                       account_directory=path_creating_folder, list_view=list_view)
