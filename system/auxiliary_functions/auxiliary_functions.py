...
def find_filess(directory_path, extension) -> list:
    ...
    :return list: Список имен найденных файлов
    """
    entities = []  # Создаем список с именами найденных файлов в директории
    for x in os.listdir(directory_path):
        if x.endswith(f".{extension}"):  # Проверяем, заканчивается ли имя файла на заданное расширение
            entities.append(x)  # Добавляем имя файла целиком (без разделения)

    logger.info(f"🔍 Найденные файлы: {entities}")  # Выводим имена найденных файлов

    return entities  # Возвращаем список найденных файлов
