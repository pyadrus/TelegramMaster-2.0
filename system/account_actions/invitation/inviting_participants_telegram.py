...
def inviting(client, phone, records, db_handler) -> None:
    ...
    for rows in records:
        ...
        try:
            ...
            record_and_interrupt("Аккаунт запущен еще на одном устройстве!", f"username : {username}", event, db_handler)
            ...
            record_and_interrupt("Предупреждение о Flood от telegram.", f"username : {username}", event, db_handler)
            break  # Прерываем работу и меняем аккаунт
            ...
            record_and_interrupt("Вам запрещено отправлять сообщения в супергруппу.", f"username : {username}", event, db_handler)
            break  # Прерываем работу и меняем аккаунт
        except ChatWriteForbiddenError:
            record_and_interrupt("Настройки в чате не дают добавлять людей в чат, возможно стоит бот админ и нужно подписаться на другие проекты ",
                                 f"username : {username}", event, db_handler)
            break  # Прерываем работу и меняем аккаунт
            ...
            record_inviting_results(username, f"username : {username}", event, f"Не корректное имя {username}", db_handler)
